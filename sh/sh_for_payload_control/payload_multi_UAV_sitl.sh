...
--tab -e 'bash -c "sleep 3; roslaunch px4_command px4_interdrone_communication_gazebo.launch; exec bash"' \
--tab -e 'bash -c "sleep 3; roslaunch px4_command px4_multidrone_pos_estimator_pure_vision.launch uavID:=uav1; exec bash"' \
--tab -e 'bash -c "sleep 3; roslaunch px4_command px4_multidrone_pos_controller_gazebo.launch uavID:=uav1; exec bash"' \
